...
test("Writer prompt forbids child Gate calls and nested task dispatch", async () => {
  ...
  const writerPrompt = agents.match(/"mm-writer":[\s\S]+?permission: actorPermissions\(reportingAttempt, ["']mm-report["']/u)
  ...
  assert.match(writerPrompt[0], /distinguish aggregate metrics from per-item or per-fold dominance/u)
})
...
test("Writer dispatcher prompt says the child must not call Gate or nested task", async () => {
  ...
  const writerPrompt = fn("writer", "case-alpha", "attempts/reporting/002/context.json", "do the work")
  ...
  const solverPrompt = fn("solver", "case-alpha", "attempts/solving/task-01/001/context.json", "do the work")
...
test("Actor tool allowlist pins exactly one mm_agent Tool per non-analyst role", async () => {
  ...
  for (const role of ["analyst", "modeler", "solver", "writer"] as const) {
    const prompt = build(role, "case-alpha", "attempts/reporting/002/context.json", "follow the contract")
  ...
  assert.match(build("analyst", "case-alpha", "attempts/reporting/002/context.json", "x"), /must not call any mm_agent Tool/u)
  assert.match(build("modeler", "case-alpha", "attempts/reporting/002/context.json", "x"), /mm_agent_hmml/u)
  assert.match(build("modeler", "case-alpha", "attempts/reporting/002/context.json", "x"), /must not call mm_agent_compute, mm_agent_compile, mm_agent_case/u)
  assert.match(build("solver", "case-alpha", "attempts/reporting/002/context.json", "x"), /Among mm_agent Tools, the child may call only mm_agent_compute/u)
  assert.match(build("solver", "case-alpha", "attempts/reporting/002/context.json", "x"), /Ordinary filesystem read, write, and shell tools remain available/u)
  assert.match(build("solver", "case-alpha", "attempts/reporting/002/context.json", "x"), /must not call mm_agent_hmml, mm_agent_compile, mm_agent_case/u)
  assert.match(build("writer", "case-alpha", "attempts/reporting/002/context.json", "x"), /mm_agent_compile/u)
  assert.match(build("writer", "case-alpha", "attempts/reporting/002/context.json", "x"), /must not call mm_agent_hmml, mm_agent_compute, mm_agent_case/u)
})
...
test("solverComputeContract pins Case-relative Tool and Task Memory paths", async () => {
  const runner = await readFile(path.join(repositoryRoot, "scripts", "run-golden-case.mjs"), "utf8")
  const base = runner.match(/function solverAttemptBase[\s\S]+?\n\}/u)
  const contract = runner.match(/const solverComputeContract = \(contextPath\) =>[\s\S]+?\n\}/u)
  assert.ok(base, "solverAttemptBase definition is present")
  assert.ok(contract, "solverComputeContract definition is present")
  const fn = new Function(`${base![0]}\n${contract![0]}\nreturn solverComputeContract;`)()
  const prompt = fn("attempts/solving/task-a/001/context.json")
  assert.match(prompt, /work_dir must be the literal string "attempts\/solving\/task-a\/001\/code"/u)
  assert.match(prompt, /Never pass an absolute path/u)
  assert.match(prompt, /execution-result\.json must be written only by mm_agent_compute/u)
  assert.match(prompt, /memory\.json execution_result must be "attempts\/solving\/task-a\/001\/execution-result\.json"/u)
})
